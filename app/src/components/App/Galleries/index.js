...
class Galleries extends Component {
    ...
    getGalleries() {
        fetch('/api/v1/galleries/all').then(response => response.json()).then(galleries => {
    ...
    render() {
        ...
        return (
            <Container className="galleries">
                {galleries.map(({uuid, galleryName, galleryDescription, releaseDate: releaseDateString, Tags: tags, Assets: assets}, i) => {
                    const releaseDate = new Date(Date.parse(releaseDateString));
                    return (
                        <NavLink to={`/gallery/${uuid}`} key={`gallery-link-${i}`} className="text-info">
                            <Card className="text-left border-primary mb-3">
                                ...
                                <Card.Img variant="top" src={`/api/v1/image/${assets[0].filename}`} />
                            </Card>
                        </NavLink>
                    );
                })}
