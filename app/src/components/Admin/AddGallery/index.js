...
import { Container, Row, Col, Button, InputGroup, FormControl, Navbar, Nav, Modal } from "react-bootstrap";
...
import { faTrash, faSortCircle, faSortCircleUp, faSortCircleDown, faCloudUpload, faCalendar, faTags, faCheckCircle, faSparkles } from '@fortawesome/pro-solid-svg-icons';
...
class AdGallery extends Component {
    ...
    async onSubmitGallery(e) {
        ...
        if (await confirm("Are your sure?", "Proceed", "Cancel", {title: "Confirm galley submission", proceedVariant: "success"})) {
            ...
            const { photos, galleryName, galleryDescription, releaseDate, tags, featuredImage, crop } = this.state;
            const upload = () => {
                const data = new FormData();
                photos.forEach(({originalWidth: width, originalHeight: height, scaleRatio, md5}, i) => {
                    data.append(`metadata[]`, `${width}:${height}:${scaleRatio}:${md5}`);
                });
                ...
                data.append('featuredImage', JSON.stringify({
                    featuredImage,
                    crop
                }));
                tags.forEach(tag => {
        ...
        Promise.all(pq).then(res => {
            ...
            res.forEach(p => {
                ...
                if (_.findIndex(photos, {md5}) === -1) {
                    const { width, height, originalWidth, originalHeight, scaleRatio, src, canvas, file } = p;
                    photos.push({
                        src,
                        canvas,
                        width,
            ...
            this.setState({
                ...
                const [width, height] = [(photo.height / max) * 100, (photo.width / max) * 100];
                this.makeClientCrop({
                    width,
                    height,
                    x: 0,
        ...
        return (
            <Container>
                            ...
                            <Nav className="mr-auto" style={{
                                ...
                                {croppedImage && (
                                    <Nav.Item style={{
                                        ...
                                        ), "OK", null, {title: "Featured Image", proceedVariant: "success"})) {
                                            // magic
                                        }
                                    }}><FontAwesomeIcon icon={faSparkles} /> Featured Image</Button></Nav.Item>
                                )}
                            </Nav>
                            <Nav style={{
                                padding: "0 5px 0 0"
                                        ...
                                        <FormControl
                                            ...
                                            onKeyDown={e => {
                                                switch (true) {
                                                    ...
                                                    case (e.key === ","): {
                                                        e.preventDefault();
                                                        if (tagInput.length === 0) { onConfirm(); return; }
                ...
                >
                    ...
                    <Modal.Footer>
                        <Button variant="success" onClick={() => this.setState({showCrop: false})}>
                            Confirm
                        </Button>
                    </Modal.Footer>
                </Modal>
